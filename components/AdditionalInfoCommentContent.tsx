...
import { addComment, removeComment } from "../state/reducers/additionalInfoSlice";
import { useAppDispatch } from "../state/hooks";
import QuestionButton from "./QuestionButton";

const AdditionalInfoCommentContent = ({ questionNumber, onDelete }: AdditionalContentProps): JSX.Element => {
  const i18n = useI18n();
  ...
  const handleRemoveComment = () => {
    dispatch(removeComment({ questionNumber }));
  };
  ...
  const handleAddComment = (e: React.KeyboardEvent<HTMLTextAreaElement>, language: string) => {
    ...
    clearTimeout(timer);
    timer = setTimeout(() => {
      dispatch(addComment({ questionNumber, language, value }));
    }, 500);
  };
    ...
    <div className={styles.maincontainer}>
      <div className={styles.contentcontainer}>
        <div className={styles.descriptioncontainer}>
          <h4>PH: voit tarkentaa esteettömyystietoja vapaamuotoisella lisätekstillä</h4>
        </div>
        <div className={styles.flexcontainer}>
          <div className={styles.commentscontainer}>
            <TextArea
              ...
              onKeyUp={(e: React.KeyboardEvent<HTMLTextAreaElement>) => handleAddComment(e, "comment_fi")}
            />
            <div className={styles.optionalaltscontainer}>
              ...
              >
                <TextArea
                  ...
                  onKeyUp={(e: React.KeyboardEvent<HTMLTextAreaElement>) => handleAddComment(e, "comment_sv")}
                />
              </QuestionInfo>
              <QuestionInfo
              ...
              >
                <TextArea
                  ...
                  onKeyUp={(e: React.KeyboardEvent<HTMLTextAreaElement>) => handleAddComment(e, "comment_en")}
                />
              </QuestionInfo>
            </div>
          </div>
